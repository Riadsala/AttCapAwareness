...
while n  < length(trialList)
    ...
    [preview, stimulus xtarg ytarg tc dc] = createStimuli(trialList, n, params);
...
sca
    ...
    function trialOK = DoATrial
        ...
        if iLink.doILink == 1
            ...
            Eyelink('Command', 'set_idle_mode'); %aoife
            WaitSecs(0.05); %aoife
        ...
        [response, ~, rt] = getObserverInput('rightarrow', 'leftarrow', params.deadline);
        Eyelink('message','RESPONSE_KEY_HIT');
        
        % Check/wait for fixation on target circle
        waitingForFixation = 1;
        
        [~,~, t0] = GetCurrentFixLoc(iLink, 0);
        while waitingForFixation
            [x y t] = GetCurrentFixLoc(iLink, t0);
            x = x - (1920-1024)/2;
            y = y - (1080-1024)/2;
            %             st = stimulus;
            %             st((y-5):(y+5), (x-5):(x+5), 1) = 0;
            %             t_st = Screen('MakeTexture', stimuliScrn, 255*st);
            %             Screen('DrawTexture', stimuliScrn, t_st);
            %             Screen('Flip', stimuliScrn);
            
            if (abs(x-ytarg)<55) && (abs(y-xtarg)<55) && (t < 1000*param.fixateDeadline)
                % We have fixated target within time limit
                waitingForFixation = 0;
                fixatedInTime = 1;
            elseif t > 1000*param.fixateDeadline;
                % taken too long
                waitingForFixation = 0;
                fixatedInTime = 0;
            end
        ...
        if (rt < params.deadline) && fixatedInTime
            trialOK = 1;
            ...
            Screen('DrawTexture', stimuliScrn, t_blank);
            DrawFormattedText(stimuliScrn, 'Was this a good eye movement: yes (up arrow) or no (down arrow)?', 'center', 'center');
            Screen('Flip', stimuliScrn);
            
            %         wait for response
            [thoughtNoAttCap, ~, TrialTypeRT] = getObserverInput('uparrow', 'downarrow', 200000);
        ...
        elseif fixatedInTime == 0
            Screen('DrawTexture', stimuliScrn, t_blank);
            DrawFormattedText(stimuliScrn, 'Remember to fixate the target', 'center', 'center');
            Screen('Flip', stimuliScrn);
            trialOK = 0;
        else
            Screen('DrawTexture', stimuliScrn, t_blank);
...
function [preview, stimulus xtarg ytarg tc dc] = createStimuli(trialList, n, params)
