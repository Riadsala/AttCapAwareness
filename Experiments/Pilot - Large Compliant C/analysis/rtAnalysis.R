...
levels(dat$thought) = c("thought captured", "thought direct")
...
adat2  = (dat 
		%>% group_by(observer, captured, congC, thought) 
		%>% summarise(
			medianRT=median(RT)))


summary(lm(medianRT ~ captured * congC * thought, adat2))
...
model <- lmer(RT ~ congC + captured + thought + (thought + congC + captured | observer), data=filter(dat, congC!="no distracter"))

model <- lmer(
	log(RT) ~ congC * captured * thought -
	congC:captured:thought +
	 (thought + congC + captured | observer), dat)